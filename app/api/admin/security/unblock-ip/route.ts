...
const schema = z.object({ ip_address: z.string().min(3) })
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const result = await IPBlockingController.unblockIP(auth.user.id, parsed.data.ip_address)
    return NextResponse.json({ success: result.success }, { status: result.success ? 200 : 404 })
  } catch (e: any) {
