/**
 * Admin Announcements - Update/Delete
 */
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const updated = await announcementsOperations.updateAnnouncement(parsedId.data.id, parsed.data)
    try {
      await SecurityLogsController.createLog({ user_id: auth.user.id, action: SecurityLogAction.ADMIN_ACTION_PERFORMED, severity: SecurityLogSeverity.LOW, description: `Updated announcement ${updated?.title}`, details: { id: updated?.id } })
    } catch {}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const ok = await announcementsOperations.deleteAnnouncement(parsedId.data.id)
    try {
      await SecurityLogsController.createLog({ user_id: auth.user.id, action: SecurityLogAction.ADMIN_ACTION_PERFORMED, severity: SecurityLogSeverity.LOW, description: `Deleted announcement id=${parsedId.data.id}`, details: { id: parsedId.data.id } })
    } catch {}
