...
async function authenticateRequest(request: NextRequest): Promise<{ success: boolean; user?: any; error?: string }> {
  try {
    ...
    // Fallback: cookie holding user data
...
export async function GET(request: NextRequest, { params }: { params: any }) {
  ...
  // Verify ownership using CythroDash server id
  ...
  try {
    const result = await ClientServerController.getServerDetails(identifier)
    if (!result.success) {
      return NextResponse.json({ success: false, message: result.message || 'Failed to fetch details' }, { status: 502 })
    }

    return NextResponse.json({ success: true, data: result.server })
  } catch (error: any) {
